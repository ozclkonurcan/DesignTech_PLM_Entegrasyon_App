...
namespace DesignTech_PLM_Entegrasyon_App.MVC.Hubs
{
    public class FormDataListHub : Hub
    {
        ...
        private readonly IGenericRepository<WTPartAlternateLink_LOG> _wTPartAlternateLink_LOG;
        public FormDataListHub(IHttpClientFactory httpClientFactory, IConfiguration configuration, IGenericRepository<Change_Notice_LogTable> change_Notice_LogTable = null, IGenericRepository<WTPartAlternateLink_LOG> wTPartAlternateLink_LOG = null)
        {
            _httpClientFactory = httpClientFactory;
            _configuration = configuration;
            _change_Notice_LogTable = change_Notice_LogTable;
            _wTPartAlternateLink_LOG = wTPartAlternateLink_LOG;
        }
        ...
        public async Task SendFormDataCount()
        {
            try
            {
                ...
                var WTPartAlternateLink_LOGList = (await _change_Notice_LogTable.GetAll(schema + ".WTPartAlternateLink_LOG")).OrderByDescending(x => x.ProcessTimestamp).ToList();
                ...
                if (Change_Notice_LogTableList is not null)
                {
                    ...
                    if (WTPartAlternateLink_LOGList is not null)
                    {
                        if (WTPartAlternateLink_LOGList.Any())
                        {
                            await Clients.All.SendAsync("AlternatePartsFormDataCount", WTPartAlternateLink_LOGList.Count);
                        }
                    }
        ...
        public async Task SendFormData()
        {
            try
            {
                ...
                //var Change_Notice_LogTableList = connection.Query<Change_Notice_LogTable>($"SELECT * FROM {schema}.Change_Notice_LogTable").OrderByDescending(x => x.ProcessTimestamp).ToList();
                var Change_Notice_LogTableList = (await _change_Notice_LogTable.GetAll(schema + ".Change_Notice_LogTable")).OrderByDescending(x => x.ProcessTimestamp).ToList();
                var WTPartAlternateLink_LOGList = (await _wTPartAlternateLink_LOG.GetAll(schema + ".WTPartAlternateLink_LOG")).OrderByDescending(x => x.ProcessTimestamp).ToList();
                ...
                if (Change_Notice_LogTableList is not null)
                {
                    ...
                    if(WTPartAlternateLink_LOGList is not null)
                    {
                        await Clients.All.SendAsync("AlternatePartsFormData", WTPartAlternateLink_LOGList);
                    }
